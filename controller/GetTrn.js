...
class GetTrn {
    async getTrn(docNum, lot) {
        ...
        if(docNum === "" && lot !== "") {
            ...
            query = `SELECT * FROM TRN_KEMENKES WHERE PENGIRIM_KODE IS NOT NULL AND PENERIMA_KODE IS NOT NULL AND KFA_CODE IS NOT NULL AND FLG_EXPORT = 'N' AND TANGGAL_TRANSAKSI = TRUNC(SYSDATE - 1) ORDER BY DOC_NUM`
        } if (lot !== "" && docNum !== "") {
